  ...
  return (
    <SkeletonGroup>
      <View style={{ padding: 16 }}>
        <Skeleton width="100%" height={520} borderRadius={24} />
        <Skeleton width="100%" height={56} borderRadius={16} delay={120} style={{ marginTop: 20 }} />
        <View style={[styles.row, { gap: 16, marginTop: 16 }]}>
          {[0, 1, 2].map((i) => <Skeleton key={i} width={0} height={52} borderRadius={16} delay={140 + i * 40} style={{ flex: 1 }} />)}
        </View>
...
export function LoyaltySkeleton() {
  return (
    <SkeletonGroup>
      <View>
        {/* Same height as the real two-line subtitle, so the card doesn't grow when the data arrives. */}
      <Skeleton width="92%" height={12} borderRadius={5} style={{ marginBottom: 4 }} />
      <Skeleton width="58%" height={12} borderRadius={5} delay={40} style={{ marginBottom: 14 }} />
